...
def detect_features(response: str) -> list[int]:

    lower = response.lower()
    return [1 if name in lower else 0 for name in CAT_LOWER]
...
for ex in tqdm(data, desc="Processing"):
    ...
    has_sld = coarse_resp.lower().startswith("y")  # Yes?
    ...
    rows.append(
        {"uid": uid, **{f"feature_{i}": features[i-1] for i in range(1, 11)}, "label": label}
    )
